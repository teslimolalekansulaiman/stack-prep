...
db-test: ## Apply every migration to a disposable database and run the schema tests
	@set -e; \
	sock="$(PWD)/.local/run"; db=sp_schema_test; \
	dropdb -h "$$sock" -p 55439 --if-exists $$db; \
	createdb -h "$$sock" -p 55439 $$db; \
	for f in database/migrations/*.sql database/tests/*.sql; do \
	  echo "  $$f"; \
	  psql -X -q -v ON_ERROR_STOP=1 -h "$$sock" -p 55439 -d $$db -f "$$f"; \
	done; \
	dropdb -h "$$sock" -p 55439 $$db; \
...
test-py: ## Run Python tests against a disposable database
	@set -e; \
	sock="$(PWD)/.local/run"; db=sp_pytest; \
	dropdb -h "$$sock" -p 55439 --if-exists $$db; \
	createdb -h "$$sock" -p 55439 $$db; \
	for f in database/migrations/*.sql; do \
	  psql -X -q -v ON_ERROR_STOP=1 -h "$$sock" -p 55439 -d $$db -f "$$f" >/dev/null; \
	done; \
	DATABASE_URL="postgresql+asyncpg://$$USER@127.0.0.1:55439/$$db" uv run pytest -q; \
	status=$$?; \
	dropdb -h "$$sock" -p 55439 $$db; \
	exit $$status
